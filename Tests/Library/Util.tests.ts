...
describe("Library/Util", () => {
    describe("#getCookie(name, cookie)", () => {
        const test = (cookie: string, query: string, expected: string) => {
            const actual = Util.getCookie(query, cookie);
        ...
        it("should parse expected input with another cookie present after", () => {
            test(
                "another=bar; ;a=testCookie=; testCookie=id|acq|renewal; other=foo|3|testCookie=",
                "testCookie",
                "id|acq|renewal"
            );
        });
    ...
    describe("#w3cTraceId()", () => {
        it("should generate a valid trace id", () => {
            const mathStub = sinon.stub(Math, "random").callsFake(() => 0);
            const expected = "00000000000040008000000000000000";
            const actual = Util.w3cTraceId();
    ...
    describe("#random32()", () => {
        const test = (i: number, expected: number) => {
            const mathStub = sinon.stub(Math, "random").callsFake(() => i);
    ...
    describe("#msToTimeSpan(totalMs)", () => {
        const test = (input: number, expected: string, message: string) => {
            const actual = Util.msToTimeSpan(input);
    ...
    describe("#validateStringMap", () => {
        it("should only allow string:string", () => {
            ...
            assert.deepEqual(
                ...
                { a: "0", b: "", c: "", d: "[]", e: "", f: "-1", g: "true", h: "false" }
            );
            assert.deepEqual(Util.validateStringMap({ d: new Date("1995-12-17T03:24:00") }), {
                d: new Date("1995-12-17T03:24:00").toJSON(),
            });
        });
        it("skips functions", () => {
        ...
        it("should handle circular references", () => {
            ...
            if (parseInt(nodeVer[0]) >= 12) {
                ...
                assert.deepEqual(Util.validateStringMap(circObj), {
                    test: "true",
                    circular:
                        "Object (Error: Converting circular structure to JSON\n    --> starting at object with constructor 'Object'\n    --- property 'circular' closes the circle)",
                    arr:
                        "Array (Error: Converting circular structure to JSON\n    --> starting at object with constructor 'Object'\n    --- property 'circular' closes the circle)",
                });
            } else {
                assert.deepEqual(Util.validateStringMap(circObj), {
                    test: "true",
    ...
    describe("#canIncludeCorrelationHeader", () => {
        it("should return true if arguments are missing", () => {
            ...
            assert.equal(
                Util.canIncludeCorrelationHeader(
                    <any>{ config: { correlationHeaderExcludedDomains: [] } },
                    null!
                ),
                true
            );
        ...
        it("should return true if domain is not on the excluded list", () => {
            ...
            const url = "http://bing.com/search?q=example.com";
        ...
        it("should return false if domain is on the excluded list", () => {
            const client = <any>{
                config: { correlationHeaderExcludedDomains: ["bing.com", "bing.net"] },
            };
            const url = "http://bing.com/search?q=node";

            assert.equal(Util.canIncludeCorrelationHeader(client, url), false);

            const urlSecure = "https://bing.com/search?q=node";

            assert.equal(Util.canIncludeCorrelationHeader(client, urlSecure), false);

            const secondDomainUrl = "http://bing.net/search?q=node";
        ...
        it("can take wildcards in the excluded domain list", () => {
            const client = <any>{ config: { correlationHeaderExcludedDomains: ["*.bing.com"] } };
            const url = "https://abc.def.bing.com";
    ...
    describe("#makeRequest()", () => {
        ...
        describe("for http request", () => {
            ...
            it("should not override options when http_proxy not defined", () => {
                ...
                Util.makeRequest(config, requestUrl, options, callback);
        ...
        describe("for https request", () => {
            ...
            it("should not override options when https_proxy not defined (custom agent - not applied)", () => {
                ...
                const config: any = { proxyHttpUrl: undefined, proxyHttpsUrl: undefined };
                Util.makeRequest(config, requestUrl, reqOptions, callback);
