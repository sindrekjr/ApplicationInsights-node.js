...
describe("AutoCollection/Exceptions", () => {
    describe("#init and dispose()", () => {
        ...
        it("disables autocollection", () => {
            const processOnSpy = sinon.spy(global.process, "on");
            const processRemoveListenerSpy = sinon.spy(global.process, "removeListener");
            ...
            if (AutoCollectionExceptions["_canUseUncaughtExceptionMonitor"]) {
                assert.equal(
                    ...
                    1,
                    "After enabling exception autocollection, there should be 1 call to processOnSpy"
                );
                assert.equal(
                    processOnSpy.getCall(0).args[0],
                    AutoCollectionExceptions.UNCAUGHT_EXCEPTION_MONITOR_HANDLER_NAME
                );
            } else {
                assert.equal(
                    processOnSpy.callCount,
                    2,
                    "After enabling exception autocollection, there should be 2 calls to processOnSpy"
                );
                assert.equal(
                    processOnSpy.getCall(0).args[0],
                    AutoCollectionExceptions.UNCAUGHT_EXCEPTION_HANDLER_NAME
                );
                assert.equal(
                    processOnSpy.getCall(1).args[0],
                    AutoCollectionExceptions.UNHANDLED_REJECTION_HANDLER_NAME
                );
            }
