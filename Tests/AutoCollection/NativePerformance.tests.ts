    ...
    if (AutoCollectNativePerformance.isNodeVersionCompatible()) {
        describe("#init and #dispose()", () => {
            it("init should enable and dispose should stop autocollection interval", () => {
                ...
                if (AutoCollectNativePerformance["_metricsAvailable"]) {
                    assert.equal(
                        ...
                        1,
                        "setInterval should be called once as part of NativePerformance initialization"
                    );
                    AppInsights.dispose();
                    assert.equal(
                    ...
                    assert.equal(
                        setIntervalSpy.callCount,
                        0,
                        "setInterval should not be called if NativePerformance package is not available"
                    );
                    AppInsights.dispose();
                    assert.equal(
                        clearIntervalSpy.callCount,
            ...
            it("constructor should be safe to call multiple times", () => {
                ...
                let native = new AutoCollectNativePerformance(client);
                const disposeSpy = sinon.spy(AutoCollectNativePerformance.INSTANCE, "dispose");

                assert.ok(native);
                assert.ok(disposeSpy.notCalled);

                assert.doesNotThrow(() => {
                    native = new AutoCollectNativePerformance(client);
                }, "NativePerformance can be constructed more than once");
                assert.ok(
                    disposeSpy.calledOnce,
                    "dispose is called when second instance is constructed"
                );
            });

            it("Calling enable when metrics are not available should fail gracefully", () => {
                const client = new TelemetryClient("1aa11111-bbbb-1ccc-8ddd-eeeeffff3333");
                const native = new AutoCollectNativePerformance(client);
        ...
        describe("#_parseEnabled", () => {
            it("should return equal input arg if no env vars are set", () => {
                assert.deepEqual(AutoCollectNativePerformance.parseEnabled(true), {
                    isEnabled: true,
                    disabledMetrics: {},
                });
                assert.deepEqual(AutoCollectNativePerformance.parseEnabled(false), {
                    isEnabled: false,
                    disabledMetrics: {},
                });

                const config = { gc: true, heap: true };
                assert.deepEqual(AutoCollectNativePerformance.parseEnabled(config), {
                    isEnabled: true,
            ...
            it("should overwrite input arg if disable all extended metrics env var is set", () => {
                ...
                assert.deepEqual(AutoCollectNativePerformance.parseEnabled({ gc: true }), {
                    isEnabled: false,
                    disabledMetrics: {},
                });
            ...
            it("should overwrite input arg if individual env vars are set", () => {
                ...
                inConfig = false;
                assert.deepEqual(AutoCollectNativePerformance.parseEnabled(inConfig), {
                    isEnabled: false,
                ...
                assert.deepEqual(AutoCollectNativePerformance.parseEnabled(<any>inConfig), {
                    isEnabled: true,
                    disabledMetrics: { ...inConfig, ...expectation },
                });
