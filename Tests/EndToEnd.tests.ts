...
class fakeResponse {
    ...
    public on(event: string, callback: () => void) {
        if (!this.callbacks[event]) {
            ...
            const lastCallback = this.callbacks[event];
...
class fakeRequest {
    ...
    constructor(private failImmediatly: boolean = true, public url?: string, responseData?: any) {
...
class fakeHttpServer extends events.EventEmitter {
    ...
    public emitRequest(url: string) {
        const request = new fakeRequest(false, url);
...
describe("EndToEnd", () => {
    ...
    describe("Basic usage", () => {
        ...
        beforeEach(() => {
            ...
            request = sandbox
                .stub(https, "request")
                .callsFake((options: any, callback: any): any => {
                    const req = new fakeRequest(false);
                    req.on("end", callback);
                    return req;
                });
        });
        ...
        it("should send telemetry", (done) => {
            ...
            const client = new AppInsights.TelemetryClient("iKey");
    ...
    describe("Disk retry mode", () => {
        ...
        beforeEach(function () {
            setupSpanExporterStub = sinon.stub(
                AppInsights.TelemetryClient.prototype,
                "setupSpanExporter"
            );
            ...
            this.statSync = sinon
                .stub(fs, "statSync")
                .returns({ isFile: () => true, size: 8000 } as any);
            lstat = sinon.stub(fs, "lstat").yields(null, { isDirectory: () => true });
            mkdir = sinon.stub(fs, "mkdir").yields(null);
            this.mkdirSync = sinon.stub(fs, "mkdirSync").returns(undefined);
            ...
            if (typeof child_process.spawnSync === "function") {
                spawnSync = sinon
                    ...
                    .returns({ status: 0, stdout: "stdoutmock" } as any);
            }
        });
            ...
            client.flush({
                callback: (response: any) => {
                    ...
                    setImmediate(() => {
                        ...
                        assert.equal(
                            writeFile.firstCall.args[2].mode,
                            0o600,
                            "File must not have weak permissions"
                        );
        ...
        it("uses WindowsIdentity to get the identity for ICACLS", function (done) {
            ...
            const client = new AppInsights.TelemetryClient("uniquekey");
            client.channel.setUseDiskRetryCaching(true);
            const origICACLS = (<any>client.channel._sender.constructor).USE_ICACLS;
            ...
            client.flush({
                callback: (response: any) => {
                    ...
                    setImmediate(() => {
                        ...
                        assert.equal(
                            spawn.firstCall.args[1][1],
                            "[System.Security.Principal.WindowsIdentity]::GetCurrent().Name"
                        );
                        assert.equal(
                            (<any>client.channel._sender.constructor).ACL_IDENTITY,
                            "stdoutmock"
                        );
        ...
        it("refuses to store data if ACL identity fails", function (done) {
            spawn.restore();
            const tempSpawn = sinon.stub(child_process, "spawn").returns({
                on: (type: string, cb: any) => {
                    if (type == "close") {
                        ...
                        client.flush({
                            callback: (response: any) => {
                                ...
                                setImmediate(() => {
                                    ...
                                    tempSpawn.restore();
                                    (<any>(
                                        client.channel._sender.constructor
                                    )).USE_ICACLS = origICACLS;
                                    done();
                                });
                            },
                        });
        ...
        it("creates directory when nonexistent", function (done) {
            lstat.restore();
            const tempLstat = sinon.stub(fs, "lstat").yields({ code: "ENOENT" }, null);
        ...
        it("does not store data when limit is below directory size", function (done) {
            ...
            const client = new AppInsights.TelemetryClient("key");
            client.channel.setUseDiskRetryCaching(true, undefined, 10); // 10 bytes is less than synthetic directory size (see file size in stat mock)
        ...
        it("cache payload synchronously when process crashes (Node >= 0.11.12)", function () {
            ...
            if (
                parseInt(nodeVer[0]) > 0 ||
                parseInt(nodeVer[1]) > 11 ||
                (parseInt(nodeVer[1]) == 11 && parseInt(nodeVer[2]) > 11)
            ) {
                const req = new fakeRequest(true);

                const client = new AppInsights.TelemetryClient("key2");
                client.channel.setUseDiskRetryCaching(true);
                ...
                assert.equal(
                    writeFileSync.firstCall.args[2].mode,
                    0o600,
                    "File must not have weak permissions"
                );
            }
        ...
        it("cache payload synchronously when process crashes (Node < 0.11.12, ICACLS)", function () {
            ...
            if (
                ...
                const client = new AppInsights.TelemetryClient("key22");
                client.channel.setUseDiskRetryCaching(true);
                const origICACLS = (<any>client.channel._sender.constructor).USE_ICACLS;
                (<any>client.channel._sender.constructor).USE_ICACLS = true; // Simulate ICACLS environment even on *nix
        ...
        it("cache payload synchronously when process crashes (Node < 0.11.12, Non-ICACLS)", function () {
            ...
            if (
                ...
                const client = new AppInsights.TelemetryClient("key23");
                client.channel.setUseDiskRetryCaching(true);
                const origICACLS = (<any>client.channel._sender.constructor).USE_ICACLS;
        ...
        it("refuses to cache payload when process crashes if ICACLS fails", function () {
            if (typeof child_process.spawnSync === "function") {
                // Doesn't exist in Node < 0.11.12
                spawnSync.restore();
                const tempSpawnSync = sinon
                    .stub(child_process, "spawnSync")
                    .returns({ status: 2000 } as any);

                const req = new fakeRequest(true);

                const client = new AppInsights.TelemetryClient("key3"); // avoid icacls cache by making key unique
                client.channel.setUseDiskRetryCaching(true);
                const origICACLS = (<any>client.channel._sender.constructor).USE_ICACLS;
                (<any>client.channel._sender.constructor).USE_ICACLS = true; // Simulate ICACLS environment even on *nix
