...
class Config {
    ...
    constructor(setupString?: string) {
        ...
        this.instrumentationKey =
            ...
            Config._getInstrumentationKey();
        this.connectionString =
            connectionStringEnv || (csCode.instrumentationkey ? setupString : undefined);
        ...
        this.endpointUrl = `${
            csCode.ingestionendpoint || csEnv.ingestionendpoint || this.endpointBase
        }/v2/track`;
        ...
        this.profileQueryEndpoint =
            ...
            process.env[Config.ENV_profileQueryEndpoint] ||
            this.endpointBase;
        this._quickPulseHost =
            csCode.liveendpoint ||
            csEnv.liveendpoint ||
            process.env[Config.ENV_quickPulseHost] ||
            Constants.DEFAULT_LIVEMETRICS_HOST;
        // Parse quickPulseHost if it starts with http(s)://
        if (new RegExp(/^https?:\/\//).exec(this._quickPulseHost)) {
            this._quickPulseHost = url.parse(this._quickPulseHost).host ?? undefined;
        }
    }

    public set profileQueryEndpoint(endpoint: string | undefined) {
    ...
    public get profileQueryEndpoint(): string | undefined {
        return this._profileQueryEndpoint;
    }

    public set quickPulseHost(host: string | undefined) {
        this._quickPulseHost = host;
    }

    public get quickPulseHost(): string | undefined {
