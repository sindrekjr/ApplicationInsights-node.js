...
const setupWarningMessage = (fn: string) =>
    `${fn}(...) was called after the provider was already set up. Provider.start() must be called again for this change to take effect.`;
...
export default class Provider {
    ...
    static setInstrumentationPlugin(
        module: string | DefaultPlugins,
        config: boolean | PluginConfig
    ): Provider {
        if (this._started) {
        ...
        if (this.config[module] && typeof config === "boolean") {
            ...
            Logging.warn("setInstrumentationPlugin(...) was called with invalid arguments", [
                module,
                config,
            ]);
        }
    ...
    static setup(): NodeTracerProvider | null {
        return this._instance;
    }
    ...
    static start(): NodeTracerProvider {
        ...
        (this.config.https as any)["ignoreOutgoingUrls"] = [
            new RegExp(/services.visualstudio.com/i),
        ];
