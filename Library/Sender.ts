...
class Sender {
    ...
    public send(payload: Buffer, callback?: (v: string) => void) {
        ...
        zlib.gzip(payload, (err, buffer) => {
            ...
            const req = Util.makeRequest(this._config, endpointUrl, options, requestCallback);
            ...
            req.on("error", (error: Error) => {
                ...
                if (
                    ...
                    (this._numConsecutiveFailures > 0 &&
                        this._numConsecutiveFailures %
                            Sender.MAX_CONNECTION_FAILURES_BEFORE_WARN ===
                            0)
                ) {
                    let notice =
                ...
                if (typeof callback === "function") {
                    let errorMessage = "error sending telemetry";
    ...
    private _runICACLSSync(args: string[]) {
        ...
        if (child_process.spawnSync) {
            const aclProc = child_process.spawnSync(Sender.ICACLS_PATH, args, <any>{
                windowsHide: true,
            });
            if (aclProc.error) {
                throw aclProc.error;
    ...
    private _applyACLRules(directory: string, callback: (err: Error | null) => void) {
        ...
        if (Sender.ACLED_DIRECTORIES[directory] === undefined) {
            ...
            return callback(
                Sender.ACLED_DIRECTORIES[directory]
                    ? null
    ...
    private _storeToDisk(payload: any) {
        ...
        const directory = path.join(
            os.tmpdir(),
            Sender.TEMPDIR_PREFIX + this._config.instrumentationKey
        );
        ...
        this._confirmDirExists(directory, (error) => {
            if (error) {
                Logging.warn(
                    Sender.TAG,
                    "Error while checking/creating directory: " + (error && error.message)
                );
                this._onErrorHelper(error);
                return;
            }
            ...
            this._getShallowDirectorySize(directory, (err, size) => {
                if (err || size < 0) {
                    Logging.warn(
                        Sender.TAG,
                        "Error while checking directory size: " + (err && err.message)
                    );
                ...
                fs.writeFile(fileFullPath, payload, { mode: 0o600 }, (error) =>
                    this._onErrorHelper(error)
                );
            });
        });
    ...
    private _storeToDiskSync(payload: any) {
        ...
        try {
            ...
            const dirSize = this._getShallowDirectorySizeSync(directory);
            ...
            Logging.warn(
                Sender.TAG,
                "Error while saving data to disk: " + (error && error.message)
            );
            this._onErrorHelper(error);
        }
    }
        ...
        fs.exists(tempDir, (exists: boolean) => {
            if (exists) {
                fs.readdir(tempDir, (error, files) => {
                    if (!error) {
                        files = files.filter((f) => path.basename(f).includes(".ai.json"));
                        if (files.length > 0) {
